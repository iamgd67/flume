...
public class KafkaSource extends AbstractSource
    ...
    private static Map<String, String> toStringMap(Map<CharSequence, CharSequence> headercs) {
        Map<String, String> mp = new HashMap();
        for (Map.Entry<CharSequence, CharSequence> en : headercs.entrySet()) {
            mp.put(en.getKey().toString(), en.getValue().toString());
        }
        return mp;
    }
                ...
                if (iterStatus) {
                    ...
                    if (useAvroEvenFormat) {
                        ByteArrayInputStream in =
                                new ByteArrayInputStream(kafkaMessage);
                        decoder = DecoderFactory.get().directBinaryDecoder(in, decoder);
                        AvroFlumeEvent avroFlumeEvent = reader.read(null, decoder);
                        event = EventBuilder.withBody(avroFlumeEvent.getBody().array(), toStringMap(avroFlumeEvent.getHeaders()));

                        log.debug("decode with header:"+event.getHeaders());
    ...
    public void configure(Context context) {
        ...
        log.info("using avroEventFormat? "+useAvroEvenFormat);
